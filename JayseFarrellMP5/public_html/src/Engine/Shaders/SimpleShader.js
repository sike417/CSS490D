...
function SimpleShader(vertexShaderPath, fragmentShaderPath) {
    ...
    this.radius = gl.getUniformLocation(this.mCompiledShader, "r");
    this.middle = gl.getUniformLocation(this.mCompiledShader, "middle");
}
//</editor-fold>
...
SimpleShader.prototype.activateShader = function (pixelColor, aCamera, radius, middle) {
    var gl = gEngine.Core.getGL();
    ...
    if(radius === undefined)
        gl.uniform1f(this.radius, .5);
    else
        gl.uniform1f(this.radius, radius / 100);
    if(middle === undefined)
        gl.uniform2f(this.middle, 0, 0);
    else
        gl.uniform2f(this.middle, middle[0], middle[1]);
